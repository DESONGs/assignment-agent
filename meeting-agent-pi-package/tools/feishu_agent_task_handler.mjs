...
function loadPublishTargetRegistry(paths) {
  ...
  if (!existsSync(path)) return { schemaVersion: "feishu-publish-target-registry-v1", entries: {} };
  try {
    const parsed = JSON.parse(readFileSync(path, "utf8"));
    return {
      schemaVersion: "feishu-publish-target-registry-v1",
      entries: parsed && typeof parsed.entries === "object" && !Array.isArray(parsed.entries) ? parsed.entries : {},
    };
  } catch {
    return { schemaVersion: "feishu-publish-target-registry-v1", entries: {} };
  }
}
...
async function publishResults(task, agentOutput, paths, options) {
  ...
  const publishKey = sessionPublishKey(task);
  const parentFolderToken = options.folderToken ?? null;
  const existingTarget = registry.entries?.[publishKey] ?? null;
  let folderToken = existingTarget?.folderToken ?? null;
  const folderName = safeSegment(`feishu-chat-${publishKey}`);
  result.publishTarget = {
    mode: "chat_session",
    sessionKey: publishKey,
    folderName,
