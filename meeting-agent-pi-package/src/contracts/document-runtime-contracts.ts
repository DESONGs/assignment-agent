...
export const DOCUMENT_WORKER_STATUSES = ["completed", "needs_fix", "blocked"] as const;
export const DOCUMENT_CHECKPOINT_STATUSES = ["running", "completed", "needs_fix", "blocked", "failed"] as const;
export const DOCUMENT_UNIT_STATUSES = ["completed", "failed"] as const;
export const DOCUMENT_CHECKPOINT_SCHEMA_VERSION = "document-workflow-checkpoint-v2" as const;
...
export const DocumentWorkUnitSchema = Type.Object({
  ...
  documentIdentityConfidence: Type.Optional(Type.Enum(["high", "medium", "low"] as const)),
...
export const DocumentWorkflowCheckpointSchema = Type.Object({
  ...
  createdAt: Type.String({ minLength: 1 }),
  updatedAt: Type.String({ minLength: 1 }),
  workflowStrategy: Type.Enum(["checkpointed", "single_pass"] as const),
  qualityMode: Type.Enum(["stable", "balanced", "fast"] as const),
