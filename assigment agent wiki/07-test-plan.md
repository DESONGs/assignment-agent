...
- `feishu-wiki-target-registry.json` 必须按 `project:{normalizedProjectTitle}`、`run:{projectKey}:{date}:{topicKey}`、`category:{projectKey}:{docType}`、`document:{sourceRun}:{docType}` 复用 Wiki 节点，不得把 `runId` 当顶层目录名。
