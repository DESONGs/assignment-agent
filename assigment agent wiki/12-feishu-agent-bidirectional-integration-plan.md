...
| 结果发布回飞书 | 已实现 Wiki 优先 + Project Workspace + Drive fallback | handler 根据 `agent-output.json` 的 QA/Policy 结果生成 `publish.json`；默认 `FEISHU_AGENT_PUBLISH_TARGET=auto`，先通过 Publish Taxonomy 生成项目树，再写 `wiki-publish-plan.json` 并用 `markdown +create`、`wiki +node-create`、`wiki +move` 发布到真实 Feishu Wiki Space；Wiki 不可用时记录 `wiki_publish_blocked_drive_fallback` 并 fallback 到 Drive。 |
| 历史发布重整 | 已实现 CLI + live 验证 | `feishu_publish_organize_cli.mjs inventory/plan/apply --live --no-delete` 已可将历史 `feishu-chat-*` 目录中的同源会议纪要、PRD、技术架构、Checklist 归并到同一项目 Wiki 树，保留旧 file token/url，不删除历史内容。 |
...
### 3.3.1 Publish Taxonomy / Project Workspace

飞书发布层不再把 `runId`、`feishu-chat-*` 或 Feishu file token 当作长期阅读入口。用户交付物发布前必须经过 Publish Taxonomy：

- 从 `documentIdentity`、`document-title-plan.json`、source H1、用户请求和人工 override 推断 `projectTitle/projectKey/sourceThreadKey`。
- 严格排除 `feishu file 00 ...`、normalized audio、generic upload filename、runId、Feishu token 作为项目名或用户可见标题。
- 默认 Wiki 路径是 `PI Agent 项目知识库 / 项目｜{projectTitle} / {会议纪要|PRD|技术架构|客户需求确认|To-do}`。
- Wiki 文档展示名格式为 `{docKind}｜{projectTitle}｜{purpose}｜{run timestamp}.md`；同源多文件必须进入同一项目树。
- Drive 只是落盘和 fallback；历史 `feishu-chat-*` 文件夹可移动/重命名，但不得删除或覆盖旧 token/url。

历史重整通过 `feishu_publish_organize_cli.mjs` 执行：`inventory` 生成清单，`plan` 生成人工可读归并方案，`apply --live --no-delete` 创建/复用 Wiki Space、项目节点、分类节点，并将本地 Markdown 重新创建为 canonical Wiki 文档。所有 live 操作写入 `publish-organization-ledger.jsonl`，并由 `feishu-wiki-target-registry.json` 保证重复运行时跳过已存在节点。
